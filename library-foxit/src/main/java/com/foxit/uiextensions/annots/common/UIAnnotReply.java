...
public class UIAnnotReply {
    ...
    public static void addReplyAnnot(final PDFViewCtrl pdfViewCtrl, final Annot annot, final PDFPage page, final String nm, final String content, final Event.Callback callback) {
        try {
            ...
            ((UIExtensionsManager) pdfViewCtrl.getUIExtensionsManager()).getDocumentManager().addAnnot(page, new ReplyContent(pageIndex, nm, content, annot.getUniqueID()), true, callback);
        } catch (PDFException e) {
            e.printStackTrace();
        }
    }
    ...
    public static class CommentsFragment extends DialogFragment {
        ...
        private void init() {
            ...
            try {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    node.setCanReply(AppAnnotUtil.isSupportReply(dmAnnot) && !_isReadOnly);
                ...
                mRootNode.setCanReply(AppAnnotUtil.isSupportReply(mAnnot) && !isReadOnly);
        ...
        class CommentsAdapter extends BaseAdapter {
            ...
            public void removeNode(final AnnotNode node) {
                ...
                try {
                    ((UIExtensionsManager) mPDFViewCtrl.getUIExtensionsManager()).getDocumentManager().removeAnnot(node.replyAnnot, true, null);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            ...
            public void flattenNode(final AnnotNode node) {
            ...
            public void redactionNode(final AnnotNode node) {
                ...
                try {
                    for (AnnotNode an : mNodesTmp) {
                        ...
                        if (an.replyToAnnot != null && !an.replyToAnnot.isEmpty()
                                && node.replyAnnot.getUniqueID().equals(an.replyToAnnot.getUniqueID())) {
                            an.setParent(node);
...
//                                                node.setCreationDate(dateTemp);
                ...
                int visibility = View.VISIBLE == holder.mLl_relist_reply.getVisibility() ||
                        ...
                        View.VISIBLE == holder.mLl_relist_delete.getVisibility() ? View.VISIBLE : View.GONE;
