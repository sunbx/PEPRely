...
public class AppAnnotUtil {
    ...
    private static final List<String> TYPES = Collections.unmodifiableList(Arrays.asList(new String[]{
            "Highlight", "Text", "StrikeOut", "Underline",
            ...
            "FreeTextCallout", "PolyLine","Audio", "Video", "Redaction"}));
    ...
    public static Annot getAnnot(PDFPage page, String UID) {
        ...
        try {
            ...
            for (int i = 0; i < nCount; i++) {
                ...
                if (annot != null && annot.getUniqueID() != null && annot.getUniqueID().compareTo(UID) == 0) {
                    return annot;
                }
            }
        } catch (PDFException e) {
...
//                    return intent == null || "FreeTextTypewriter".equals(intent);
    ...
    public static Annot createAnnot(Annot annot, int type) {
        ...
        try {
            if (object != null && !object.isEmpty() && object.getUniqueID() == null)
                object.setUniqueID(AppDmUtil.randomUUID(null));
        } catch (PDFException e) {
            e.printStackTrace();
        }
        return object;
