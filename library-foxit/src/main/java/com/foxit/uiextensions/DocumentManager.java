...
public class DocumentManager extends AbstractUndo {
    ...
    public void setCurrentAnnot(Annot annot, boolean reRender) {
        ...
        try {
            ...
            if (annot != null && uiExtensionsManager.getAnnotHandlerByType(AppAnnotUtil.getAnnotHandlerType(annot)) != null) {
                ...
                uiExtensionsManager.getAnnotHandlerByType(AppAnnotUtil.getAnnotHandlerType(annot)).onAnnotSelected(annot, reRender);
            }
    ...
    public boolean canPrint() {
        ...
        if (mIsCDRM) {
            return (mCDRMPermission & PDFDoc.e_PermPrint) == PDFDoc.e_PermPrint;
        }
    ...
    public boolean canPrintHighQuality() {
        ...
        if (mIsCDRM) {
            return (mCDRMPermission & PDFDoc.e_PermPrint) == PDFDoc.e_PermPrint ||
                    (mCDRMPermission & PDFDoc.e_PermPrintHigh) == PDFDoc.e_PermPrintHigh;
        }
    ...
    public boolean canCopy() {
        ...
        if (mIsOwner) return true;
        if (mIsCDRM) {
            return (mCDRMPermission & PDFDoc.e_PermExtract) == PDFDoc.e_PermExtract;
        }
    ...
    public boolean canCopyForAssess() {
        ...
        if (mIsOwner) return true;
        if (mIsCDRM) {
            return (mCDRMPermission & PDFDoc.e_PermExtractAccess) == PDFDoc.e_PermExtractAccess
                    || (mCDRMPermission & PDFDoc.e_PermExtract) == PDFDoc.e_PermExtract;
        }
    ...
    public boolean canAssemble() {
        ...
        if (mIsCDRM) {
            if (mIsOwner) return true;
            return (mCDRMPermission & PDFDoc.e_PermAssemble) == PDFDoc.e_PermAssemble
                    || (mCDRMPermission & PDFDoc.e_PermModify) == PDFDoc.e_PermModify;
        }
    ...
    private boolean canModifyContents(boolean isJudgeAddSignature) {
        ...
        if (mIsCDRM) {
            if (mIsOwner) return true;
            return (mCDRMPermission & PDFDoc.e_PermModify) == PDFDoc.e_PermModify;
        }
    ...
    public boolean canFillForm() {
        ...
        if (mIsCDRM) {
            if (mIsOwner) return true;
            return (mCDRMPermission & PDFDoc.e_PermFillForm) == PDFDoc.e_PermFillForm
                    || (mCDRMPermission & PDFDoc.e_PermAnnotForm) == PDFDoc.e_PermAnnotForm
                    || (mCDRMPermission & PDFDoc.e_PermModify) == PDFDoc.e_PermModify;
        }
    ...
    public boolean canAddAnnot() {
        ...
        if (mIsCDRM) {
            if (mIsOwner) return true;
            return (mCDRMPermission & PDFDoc.e_PermAnnotForm) == PDFDoc.e_PermAnnotForm;
        }
...
//        if (isSign()) return false;
    ...
    public Annot getAnnot(PDFPage page, String nm) {
        ...
        try {
            ...
            for (int i = 0; i < count; i++) {
                annot = AppAnnotUtil.createAnnot(page.getAnnot(i));
                if (annot == null || annot.isEmpty() || annot.getUniqueID() == null) continue;
            ...
            switch (action) {
                case MotionEvent.ACTION_DOWN: {
                    ...
                    if (viewCtrl != null){
                        ...
                        if (annot != null && !annot.isEmpty()) {
                            boolean isReadOnly = (annot.getType() == Annot.e_Widget) ? FormFillerUtil.isReadOnly(annot) : AppAnnotUtil.isReadOnly(annot);
                            if (AppAnnotUtil.isLocked(annot) || isReadOnly || !canAddAnnot()) return false;
...
//                    isViewSignedDoc = false;
    ...
    private ArrayList<Integer> mAccessPages = new ArrayList<>();
    private String mWrapperContent;
    void parseACL(String acl) {
        mCDRMPermission = 0xffffffff;
        mIsAccessPages = false;
        mAccessPages.clear();
        try {
    ...
    public void onAnnotWillFlatten(int index, String uniqueID) {
